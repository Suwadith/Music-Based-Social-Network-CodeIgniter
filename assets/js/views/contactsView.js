var ContactItemsView = Backbone.View.extend({
    ...
    onClickSearch: function() {
        ...
        if(lastNameBox.val() !== '' || relationalTagBox.val() !== ''){
            ...
            var nameArr = lastNameBox.val().split(' ');
            var tagArr  = relationalTagBox.val().split(',');
            // console.log(arr);
            contacts.fetch({
                data: {
                    // lastName: JSON.stringify(nameArr),
                    lastName: lastNameBox.val(),
                    relationalTag: JSON.stringify(tagArr)
                },
